...
| Major | M-02 数据刷新链路契约化 | **当前任务** | 先补事件→副作用矩阵，再提取纯决策函数 |
...
1. **M-02.1 事件→副作用矩阵（当前）**：为 `friendPresenceCoordinator` 覆盖 Online/Offline 的状态、Feed、通知、共享 Feed、数据库写入和排序更新。
2. **M-02.2 纯 diff/记录决策函数**：只提取输入→Feed 决策，不改变调用顺序和写入时机。
