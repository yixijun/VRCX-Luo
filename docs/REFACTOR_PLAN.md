...
2. **M-00.2 现有类型债务分批收敛（进行中）**：工具链启用后，当前检查暴露 210 条既有诊断，已通过低风险契约切片降至 52 条；主要剩余项集中在 API response 类型、宿主 bridge 类型、store 推断和旧 JSDoc。按模块 seam 分批处理，每批保持定向测试和生产构建通过。
