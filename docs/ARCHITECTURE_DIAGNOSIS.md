...
| `src/stores/ui.js` | store 同时管理状态、router、DOM drop 事件、开发者工具和窗口行为 | Major → 已完成 M-11.2～M-11.4 | body `drop` guard、对话框面包屑状态和托盘通知双宿主路由已移出；剩余 router/窗口动作刻意留在 Store，前端本轮达到停止线，后续以 VR CEF 问题验证为主 |
