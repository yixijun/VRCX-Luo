...
> 分析范围：当前工作树中的 Vue 3、Pinia、Electron、CEF/C#、数据库模块、测试和项目文档。本文记录诊断结论和实施状态；updateLoop 第一刀、M-01.1 剪贴板 capability、M-01.2a/2b 文件与目录选择 capability、M-01.3 Dotnet bridge allowlist、B-01 宿主桥安全封口、M-03 编排层适配器及 callable interface 修复、M-06 Notification Store 低风险 seam、M-08 API/Query 缓存 seam、M-09 Group/Favorite/User 纯 module 切片、M-07.1～M-07.8 Electron composition-root seam、M-00.1～M-00.3 测试/CI 门禁、N-02 版本来源与一致性门禁，以及 N-03 Shared/Localization 依赖和语言包门禁已落地，其余内容仍是只读建议。

> 实施状态（2026-09-05）：`updateLoop` 调度器、独立任务 module、M-01.1 的 CEF/Electron 剪贴板 capability、M-01.2a/2b 的 CEF/Electron 文件与目录选择 capability、M-01.3 的 Dotnet bridge class/method allowlist 与 args envelope 校验、B-01 的可信渲染来源 guard 与 174 个方法参数 schema、M-03 的 UI adapter 与 callable Toast interface、M-06 的通知 projection/persistence/seen queue module、M-08 的 Query cache/resource registry seam、M-09.1～M-09.8 的 Group/Favorite/User 纯决策与 projection module、M-07.1～M-07.8 的 Electron .NET bootstrap/IPC 注册/双宿主 capability contract/窗口状态/窗口事件/窗口关闭守卫/托盘菜单/托盘图标工厂 Adapter、M-00.1～M-00.3 的 JavaScript 类型收敛/重构 smoke/分阶段 CI 门禁、N-02 的共享版本元数据/package-lock 同步/一致性校验，以及 N-03 的 Shared/Localization 依赖方向 guard/语言包 contract/CI 聚合门禁均已落地；coordinator/store 公共 interface 保持不变，`main.js` 的 tray lifecycle/notification 实现仍未迁移，多账户运行时功能仍按要求暂停。
...
| `src-electron` | 主进程、窗口、托盘、IPC、C# 启动 | `main.js` 约 1007 行，托盘生命周期、通知和生命周期仍集中；M-07.1 已把 10 个同步 .NET 初始化调用移入 `dotnetBootstrap.cjs`，M-07.2 已把 IPC channel 注册移入 `ipcHandlers.cjs`，M-07.3 已固化双宿主 capability contract，M-07.4～M-07.6 已移出窗口状态、窗口事件和关闭守卫，M-07.7 已移出托盘上下文菜单，M-07.8 已移出平台图标选择/Tray 创建，B-01 新增 renderer source policy 与统一 IPC guard |
...
| `src-electron/main.js` | 主进程约 1007 行，窗口、托盘、通知、IPC、Dotnet 启动集中 | Major → M-07.1～M-07.8 首批 seam 完成 | Dotnet 启动、IPC 注册、窗口状态读取/动作、窗口事件、关闭守卫、托盘菜单和图标创建已移出，双宿主 contract 已可执行；tray lifecycle、notification 行为实现仍可独立拆分 |
...
- M-07.1～M-07.8 已提取 Electron composition-root 首批 seam：`dotnetBootstrap.cjs` 通过显式依赖执行原有 10 个同步初始化调用；`ipcHandlers.cjs` 集中 15 个 IPC channel 注册；`hostCapabilityContract.js` 固化七类 CEF/Electron capability 的 method、参数、channel 与取消/错误语义；`windowState.cjs`、`windowEventBridge.cjs`、`windowCloseHandler.cjs`、`trayContextMenu.cjs` 和 `trayIconFactory.cjs` 分别承接窗口状态、窗口事件、关闭守卫、托盘菜单和平台图标/Tray 创建。所有改动均保留原顺序、参数、同步/异常行为，窗口、托盘图标/生命周期、VR overlay 与 renderer interface 未改变。
...
| M-07.1～M-07.8 Electron composition root / 双宿主 contract | IPC 注册、双宿主契约、窗口状态/事件/关闭守卫/托盘菜单/图标工厂定向测试通过；与 M-01 capability 回归合计 11 个文件、34 个测试通过；格式、Node 语法和生产构建通过 |
