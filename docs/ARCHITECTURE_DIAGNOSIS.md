...
| `npm run typecheck:js` | 工具链已补齐并可执行；当前仍有 188 条既有 TypeScript/JavaScript 诊断，已先消除 `updateLoop.js(65,28)`、Group API `bool`、Avatar 空参数、Notification typedef、V2 projection 误标参数、Feed 差异函数旧参数名、邀请 `rsvp` 误标参数、13 个上传选项误报及 World API `ref` 推断误报，尚未作为阻断式 CI 门禁 |
