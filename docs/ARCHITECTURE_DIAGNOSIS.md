...
| `src/stores/ui.js` | store 同时管理状态、router、DOM drop 事件、开发者工具和窗口行为 | Major → 已完成 M-11.2 | body `drop` guard 已移入 `src/services/dropGuard.js`；后续再拆 UI state 与 window actions，保持 Store public interface |
