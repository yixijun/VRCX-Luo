...
- 腕部 Overlay 的鼠标缩放固定为 `512×512`，共享纹理中的手背裁剪区为 `u=0..0.5`、`v=0..1/3`。OpenVR 文档定义 `ComputeOverlayIntersection` 返回 Overlay 像素坐标，但部分 SteamVR/runtime 组合会返回 Overlay UV，或直接返回共享纹理的 UV/像素坐标；边界函数会把这些格式统一还原为手背本地 `0..1` 坐标，再将 OpenVR 的左下角原点转换为 DOM 的左上角坐标。首次采样落在左上角的单位范围时存在像素/UV 歧义，后续一旦出现越界证据会一次性恢复到正确坐标空间，避免指针被锁在共享纹理左上角的小区域。Vue 端最终将其限制到 `0..1`，以百分比移动实心原点标记。
- 当 SteamVR 短暂返回“未命中”（通常是射线跨过 Overlay 边缘的一两个采样）时，CEF/Electron 会在 `100ms` 内保持上一次有效点；超过窗口或 Overlay 本身隐藏后才隐藏指针。保持窗口只用于连续显示，点击仍要求当前帧真实命中，原有点击边沿规则不变。
- 原点标记不使用 CSS 位置过渡，直接跟随宿主采样；这样不会在约 `32ms` 的宿主更新间隔上叠加更长的动画而产生追赶式跳跃。按压态的缩放也即时切换。
...
问题表现为桌面测试页只有空的手背外壳和时间栏。原因是静态页没有 CEF/OpenVR 宿主去触发正式版的 `vrInit()` 数据链路，因而 `config`、好友动态、位置、设备和在线人数都保持默认空值。

修正方式是在 `wrist-pointer-test=1` 分支内使用同一组 `$vr.*Update` 公共入口注入确定性快照，并让安全替身返回头显、左右手柄和基站状态。正式 CEF/Electron 绑定、VR 模板、Overlay 尺寸和生产路径均未修改。桌面页仍保留正式的 1024×1536 共享画布：左上 512×510 是手背裁剪区，下面的 1024×1024 是 HMD 裁剪区，其余空白属于未被 VR Overlay 使用的纹理区域。

本次修正的独立回滚点为 `1f6454ea` 和 `57288b01`，未发布、未推送。定向测试 18/18 通过，JavaScript 类型检查、目标文件 Lint、生产构建和浏览器手动验证均通过；手动验证确认快照数据可见且原点可移动。
