...
- 射线优先采用 OpenVR 渲染模型的 `tip` 组件（SteamVR 定义的瞄准/指针姿态）：先把 `tip` 的起点和局部 `-Z` 方向变换到当前控制器的追踪空间，再通过 `ComputeOverlayIntersection` 求出手背 Overlay 上的 UV 坐标。
- 若运行时或控制器模型不提供 `tip` 组件，则回退到控制器整体的局部 `-Z` 射线，保证旧设备仍可使用。
- OpenVR 的 UV 原点在左下角，送入 DOM 前转换为左上角坐标；Vue 端将其限制到 `0..1`，以百分比移动实心原点标记。
...
- `tip` 组件来自当前控制器的渲染模型，因此起点和方向会随手柄型号自动匹配；仍需要在实际 SteamVR 设备上确认不同控制器型号的射线方向、Overlay 尺寸和扳机阈值，这属于硬件验证，不在单元测试可覆盖范围内。
