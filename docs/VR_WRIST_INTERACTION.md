...
- 腕部 Overlay 的鼠标缩放固定为 `512×512`。OpenVR 文档定义 `ComputeOverlayIntersection` 返回该缩放下的像素坐标，但部分 SteamVR 运行时在同样设置下会返回 `0..1` 的 UV；边界函数对整对单位范围值保留 UV、对更大值按 `512×512` 像素归一化，随后将 OpenVR 的左下角原点转换为 DOM 的左上角坐标。这样不会把运行时的 UV 再除以 512 而锁死在左上角。Vue 端最终将其限制到 `0..1`，以百分比移动实心原点标记。
