...
- 腕部 Overlay 的鼠标缩放固定为 `512×512`。OpenVR 的 `ComputeOverlayIntersection` 返回该缩放下的像素坐标（包括小于 1 的亚像素值），边界函数统一按 `512×512` 归一化，随后将 OpenVR 的左下角原点转换为 DOM 的左上角坐标。Vue 端最终将其限制到 `0..1`，以百分比移动实心原点标记。
