...
- 腕部 Overlay 的鼠标缩放固定为 `512×512`。`ComputeOverlayIntersection` 在不同 SteamVR/runtime 组合下可能返回归一化 UV 或该缩放下的像素坐标：边界函数会识别 `0..1` 的 UV，避免重复除以 512；像素值再按该范围归一化，随后统一将 OpenVR 的左下角原点转换为 DOM 的左上角坐标。Vue 端最终将其限制到 `0..1`，以百分比移动实心原点标记。
