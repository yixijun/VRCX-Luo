...
以 `upstream-v2026.09.16` 为上游基线，将与本地产品方向一致的功能性更新合并到本地 UI 和现有服务边界中。保留本地已有的 CEF、VR 手背/HUD、托盘通知、任务栏、房间进出筛选记忆、动画和双人关系统计实现；没有直接覆盖本地的宿主层或整页重构。

本次合并包含：

- 用户资料新接口与 `iconUrl` 优先链路：首次打开资料时提前抓取公开资料，头像历史兼容旧字段，用户 JSON 刷新键修正。
- 资料外观能力：资料背景、主题、特效和图标框；好友位置、侧栏和紧凑好友列表增加独立的特效显示开关。
- 群组能力：群组编辑、转移、活动创建/编辑/重复活动、活动公告订阅，以及群组相册/实例公告入口。
- 实例与房间：实例公告、年龄限制/队列提示、群组名称排除、当前实例世界名称符号修正。
- 通知与管理：通知红点初始化和重连刷新、好友申请自动拒绝、审核记录批量清理、房间进出接口的 429 处理。
- 搜索与收藏：本地好友收藏搜索结果复用统一卡片、头像/世界/打印收藏索引、打印收藏保留策略、库存类型筛选。
- 兼容性修正：WebSocket 关闭原因记录、迁移重复显示名清理、游戏日志删除后的响应式刷新、外链复制/预览弹窗。

### 界面本地化与视觉统一

- 群组编辑、转移、活动/实例公告、用户资料编辑、相册筛选、打印收藏、收藏分组、审核确认和双人地图排行等上游界面已接入现有语言包；资料背景名称、身份组筛选和表情保存错误提示不再直接显示英文。
- 弹窗统一使用 `x-dialog` 外层样式；卡片统一使用主题 `lg` 圆角，输入/列表/缩略图使用 `md` 内层圆角，图片继承容器圆角，移除资料弹窗中混用的 `xl`、15px 和不对称圆角。
- 保留本地 VR/托盘/任务栏/动画等产品界面，不改变现有功能入口和数据协议。
...
| `npm run check:localization` | **通过：14 个语言包、2955 个规范键** |
| `npm run test:refactor -- --reporter=dot` | **通过：77 个文件、378 个测试** |
| `npx vitest run src/api/__tests__/userProfile.test.js src/services/__tests__/localizationContract.test.js src/shared/utils/__tests__/appActions.test.js --reporter=dot` | **通过：3 个文件、13 个测试** |
| `npm run prod` | **通过：4441 个模块**；仅保留既有动态导入与 Node deprecation 提示 |
| `git diff --check` | **通过** |

本地测试版已用独立配置副本启动：`build/Cef/VRCX-Luo.exe --config=<临时测试目录>`，主进程 PID 为 20980，窗口标题为 `VRCX-Luo Nightly Build`，主窗口响应正常。正式安装版未停止、未覆盖。由于正式安装版仍占用 Overlay IPC 的 `127.0.0.1:34582`，本次并行启动只验证了 CEF 页面和功能构建，VR Overlay 需在关闭正式安装版后单独启动测试版再验证。

## 资料页兼容合并修正：保留简介 diff 与本地资料操作

2026-09-17，修正资料弹窗上游 UI 合并后出现的两个回归：`current_instance`、`vrcx_info` 等键直接显示为原始 key，以及本地简介 diff/历史入口消失。当前实际渲染的 `UserDialogInfoTab.vue` 现在同时支持新公开资料字段和旧缓存字段（`publicProfileRef` / `ref` 回退），恢复简介 diff 切换、简介历史弹窗，并保留本地的头像复制、戳一戳、互关/Discord 连接隐私开关、用户 ID 复制菜单和 BioDialog 入口。资料页仍使用上游的卡片布局、实例信息和资料外观能力；这次是兼容合并，不是以单一上游组件覆盖本地功能。

| 检查项 | 结果 |
|---|---|
| 资料页定向回归 | **通过：3 个文件、18 个测试**；覆盖实际 `UserDialogInfoTab.vue`、简介历史 diff 工具和语言包契约 |
| `npm run typecheck:js` | **通过：0 diagnostics** |
| `npm run check:localization` | **通过：14 个语言包、2958 个规范键**；资料页中英文 key 均存在 |
| `npm run prod` | **通过：4442 个模块**；保留既有 Vite 动态导入和 Node deprecation 提示 |
| 本地测试版 | **已重启：PID 43964，窗口标题 `VRCX-Luo Nightly Build`，Responding=True；未发布、未推送** |
