...
<template>
    <AlertDialogPortal :to="portalTo">
        ...
        <AlertDialogContent
            ...
            :class="
                cn(
                    'bg-background data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 fixed top-[50%] left-[50%] z-11000 grid w-full max-w-[calc(100%-1.5rem)] translate-x-[-50%] translate-y-[-50%] gap-4 rounded-lg border border-border/80 p-5 shadow-xl duration-150 sm:max-w-lg max-h-[calc(100dvh-1.5rem)] min-w-0 overflow-y-auto',
                    props.class
                )
            ">
            <slot />
