                                        ...
                                        <div class="flex w-full items-center justify-between gap-3">
                                            <span class="inline-flex shrink-0 items-center gap-1">
                                                <span
                                                    ...
                                                    :title="t('dialog.edit_profile.button_color')"></span>
                                                <span
                                                    class="size-3 rounded-sm border"
                                                    :style="{ backgroundColor: getThemeHexColor(option.iconColor) }"
                                                    :title="t('dialog.edit_profile.icon_color')"></span>
                                                <span
                                                    class="size-3 rounded-sm border"
                                                    :style="{ backgroundColor: getThemeHexColor(option.subtextColor) }"
                                                    :title="t('dialog.edit_profile.subtext_color')"></span>
                                            </span>
                                    ...
                                    <template v-if="selectedProfileBackgroundTextureOption">
                                        <span class="inline-flex min-w-0 items-center gap-2">
                                            <img
                                                ...
                                                :alt="getProfileBackgroundLabel(selectedProfileBackgroundTextureOption)"
                                                loading="lazy" />
                                            <span class="truncate">{{
                                                getProfileBackgroundLabel(selectedProfileBackgroundTextureOption)
                                            }}</span>
                                        </span>
                                    </template>
                                </SelectValue>
                            ...
                            <SelectContent class="w-80">
                                <SelectGroup>
                                    <SelectItem
                                        ...
                                        :key="option.id"
                                        :value="option.id"
                                        :text-value="getProfileBackgroundLabel(option)"
                                        :disabled="!isLocalUserVrcPlusSupporter && option.isVRCPlus">
                                        <div class="flex items-center gap-2">
                                            <img
                                                :src="option.thumbnail"
                                                class="h-9 w-16 shrink-0 rounded-sm object-cover"
                                                :alt="getProfileBackgroundLabel(option)"
                                                loading="lazy" />
                                            <span>{{ getProfileBackgroundLabel(option) }}</span>
                                        </div>
                                    </SelectItem>
                                </SelectGroup>
                            </SelectContent>
...
<script setup>
    ...
    function getProfileBackgroundLabel(option) {
        if (!option) {
            return '';
        }
        const key = `dialog.edit_profile.backgrounds.${option.id}`;
    ...
    function handleCreateNewTheme() {
        ...
        modalStore
            .prompt({
                ...
                description: t('dialog.edit_profile.theme_name_description'),
                confirmText: t('dialog.edit_profile.create_theme'),
                cancelText: t('dialog.alertdialog.cancel'),
                pattern: /^.{1,12}$/,
                errorMessage: t('dialog.edit_profile.theme_name_error')
            })
            .then(({ ok, value }) => {
                if (!ok) {
                    return;
    ...
    async function saveProfile() {
        ...
        try {
            ...
            toast.success(t('dialog.edit_profile.profile_updated'));
        } finally {
            D.loading = false;
        }
    }
