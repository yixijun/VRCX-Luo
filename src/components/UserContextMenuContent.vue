<template>
    <ContextMenuContent>
        ...
        <ContextMenuSeparator v-if="hasMainActions || $slots.append" />
        <ContextMenuItem v-if="isOnline && !inCurrentInstance" @click="handleRequestInvite">
            <Mail class="size-4" />
        ...
        <ContextMenuItem
            v-if="isGameRunning && !inCurrentInstance"
        ...
        <ContextMenuItem v-if="!isCurrentUser" :disabled="!currentUser?.isBoopingEnabled" @click="handleSendBoop">
            <Hand class="size-4" />
            {{ t('dialog.user.actions.send_boop') }}
        </ContextMenuItem>
        <ContextMenuSeparator v-if="isOnline && hasLocation && !inCurrentInstance" />
        <ContextMenuItem v-if="isOnline && hasLocation && !inCurrentInstance" :disabled="!canJoin" @click="handleJoin">
            <LogIn class="size-4" />
            {{ t('dialog.user.info.launch_invite_tooltip') }}
        </ContextMenuItem>
        <ContextMenuItem
            v-if="isOnline && hasLocation && !inCurrentInstance"
            :disabled="!canJoin"
            @click="handleSelfInvite">
            <Mail class="size-4" />
            {{ t('dialog.user.info.self_invite_tooltip') }}
        </ContextMenuItem>
        <ContextMenuSeparator v-if="$slots.append && hasMainActions" />
        <slot name="append" />
    </ContextMenuContent>
</template>
...
<script setup>
    ...
    const hasMainActions = computed(
        () =>
            !isCurrentUser.value ||
            (!props.inCurrentInstance && (isOnline.value || isGameRunning.value || hasLocation.value))
    );
